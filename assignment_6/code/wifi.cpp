...
nsapi_size_t connect_to(char* web, NetworkInterface *network, SocketAddress &adr){
...
nsapi_size_t send_info(const char *request){
    if (response != NSAPI_ERROR_OK) {           
...
nsapi_size_t read_info(char *buffer, size_t size_of_buffer){
    ...
    while (expected_bytes > 0) {
        response = sct->recv(buffer + presented_bytes, 100);
